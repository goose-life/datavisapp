...
def make_table(db, table_name, col_types):
    ...
    Args:
        db (:obj: `records.DataBase`): records.Database SQLite object.
...
def create_db(db, schema_json):
    """Create a database according to a schema in JSON format.

    Args:
        db (:obj: `records.DataBase`): records.Database SQLite object.
        schema_json (Union[str, Path]): path to the JSON schema file.
...
def main(metadata_json, results_csv, db_path, schema_json):
    ...
    try:
        add_data_to_db(db, metadata_json, results_csv)
